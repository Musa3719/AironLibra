    ...
    public static void ArrangeLookAtForCamPosition(Player pl, bool isFromRangedWeapon = false)
    {
        ...
        else
        {
            ...
            if (!M_Input.IsLastInputFromGamepadForAim())
            {
                ...
                {
                    Vector3 distance = pl._RayForLook.point - pl.transform.position;
                    distance.y = 0f;
            ...
            if (isFromRangedWeapon && Physics.Raycast(ray, out pl._RayForLook, 100f, GameManager._Instance._TerrainSolidWaterHumanMask, QueryTriggerInteraction.Collide))
            {
                ...
                GameManager._Instance._RangedAimMesh.transform.position = pl._RayForLook.collider == null ? GamepadMouse._Instance.GetRangedAimPoint(true) : pl._RayForLook.point;
                GameManager._Instance._RangedAimMesh.transform.forward = pl._RayForLook.collider == null ? GamepadMouse._Instance.GetRangedAimNormal() : pl._RayForLook.normal;
            }
        }

        if (pl._LastJumpedTime + 0.85 > Time.timeAsDouble) { pl._LookAtForCam.transform.position = new Vector3(pl._LookAtForCam.position.x, pl._LastJumpedPosition.y, pl._LookAtForCam.position.z); return; }
    }
