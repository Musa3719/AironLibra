...
public static class MovementStateMethods
{
    ...
    public static void ControlRotationType(Humanoid human, float rotationSpeed = 0f)
    {
        ...
        {
            //Vector3 aimTargetDirection = (human is Player pl) ? (pl._LookAtForCam.position - human._Transform.position).normalized : ((human as NPC)._AimPosition - human._Transform.position).normalized;
            Vector3 aimTargetDirection = (human._AimPosition - human._Transform.position).normalized;
            Vector2 first = new Vector2(human._Transform.forward.x, human._Transform.forward.z);
            ...
            //if (human._IsAttacking || human.GetVelocity().magnitude >= 0.1f || angle > 25f || human._LastTimeRotated + 0.25f > Time.time)
            RotateToDirection(aimTargetDirection, human, rotationSpeed);

            if (angle > 4f && human.GetVelocity().sqrMagnitude < 0.01f)
    ...
    public static void AttackMove(Humanoid human)
    {
        ...
        Vector3 dir = human._Transform.forward;
        float attackMoveSpeed = Mathf.Clamp(human._AttackReadyTime < 0.35f ? 0.25f : human._AttackReadyTime, 0f, 0.7f) * 6f;
        human.SetVelocity(Vector3.ProjectOnPlane(dir, human._LocomotionSystem.groundHit.normal).normalized * attackMoveSpeed, true);
    ...
    {
        Vector2 first = new Vector2(human.GetVelocity().x, human.GetVelocity().z).normalized;
        Vector2 second = new Vector2(human._Transform.forward.x, human._Transform.forward.z).normalized;
        return Vector2.Dot(first, second) > 0.85f;
    }
    public static bool IsInDeepWater(Humanoid human)
    {
        float yLevel = Mathf.Max(human._Transform.position.y + 1f, WorldHandler._Instance._SeaLevel + 0.5f);
        if (Physics.Raycast(new Vector3(human._Transform.position.x, yLevel, human._Transform.position.z), -Vector3.up, out RaycastHit hit, 1f, GameManager._Instance._TerrainSolidWaterMask))
    ...
    private static void CheckSprintForPlayer(bool runInput, bool sprintInput, Player player)
    {
        ...
        bool isVelocityEnoughForSprint = player.GetVelocity().magnitude > (player._IsStrafing ? player._LocomotionSystem.AimingMovementSetting.runningSpeed : player._LocomotionSystem.FreeMovementSetting.runningSpeed) - 2f;
    ...
    private static void CheckSprintForNPC(bool runInput, bool sprintInput, NPC npc)
    {
        ...
        if (npc._MovementState is SwimMoveState)
        {
            ...
            sprintConditions = npc._DirectionInput.sqrMagnitude > 0.1f && CanRunWithStamina(npc) && IsMovingForward(npc) && !npc._HealthSystem._IsUnhealthy && new Vector3((npc._LastCornerFromPath - npc._Transform.position).x, 0f, (npc._LastCornerFromPath - npc._Transform.position).z).magnitude > 0.7f;
            if ((sprintInput || runInput) && sprintConditions)
        ...
        if ((sprintInput || runInput) && sprintConditions)
        {
            bool isVelocityEnoughForSprint = npc.GetVelocity().magnitude > (npc._IsStrafing ? npc._LocomotionSystem.AimingMovementSetting.runningSpeed : npc._LocomotionSystem.FreeMovementSetting.runningSpeed) - 2f;
            if (sprintInput && isVelocityEnoughForSprint)
            {
                npc._LocomotionSystem.FreeMovementSetting.walkByDefault = false;
            }

            if (npc.GetVelocity().sqrMagnitude > 0.1f && !npc._IsSprinting && sprintInput)
            {
                npc._IsSprinting = true;
            }
    ...
    public static void Dodge(Humanoid human)
    {
        ...
        Vector3 dir = human._DirectionInput.magnitude > 0.1f ? GameManager._Instance.Vector2ToVector3(human._DirectionInput) : -human._Transform.forward;
        human.SetVelocity(Vector3.ProjectOnPlane(dir, human._LocomotionSystem.groundHit.normal).normalized * human._LocomotionSystem.dodgeSpeed, true);
    ...
    private static void Jump(Humanoid human)
    {
        ...
        human._JumpStartSpeed = new Vector2(human.GetVelocity().x, human.GetVelocity().z).magnitude;

        var vel = human.GetVelocity();
        vel.y = human._LocomotionSystem.jumpHeight;
        human.SetVelocity(vel, true);

        // trigger jump animations
        float flatVelMagnitude = new Vector3(human.GetVelocity().x, 0f, human.GetVelocity().z).magnitude;
    ...
    public static void AirControl(Humanoid human)
    {
        ...
        if (human._Transform.position.y > human._LocomotionSystem.heightReached) human._LocomotionSystem.heightReached = human._Transform.position.y;
        ...
        Vector3 targetPosition = human._Transform.position + (human._LocomotionSystem.moveDirection * human._LocomotionSystem.airSpeed * (human._JumpStartSpeed + 1.4f) * 6f) * Time.fixedDeltaTime;
        Vector3 targetVelocity = (targetPosition - human._Transform.position) / Time.fixedDeltaTime;

        targetVelocity.y = human.GetVelocity().y;
        human.SetVelocity(Vector3.Lerp(human.GetVelocity(), targetVelocity, human._LocomotionSystem.airSmooth * Time.fixedDeltaTime), false);
    }
    ...
    public static void CheckGround(Humanoid human)
    {
        ...
        if (human._LocomotionSystem.groundDistance <= human._LocomotionSystem.groundMinDistance)
        {
            ...
            //human._Rigidbody.AddForce(human._Transform.up * (human._LocomotionSystem.extraGravity * 2 * Time.fixedDeltaTime), ForceMode.VelocityChange);

            human._LocomotionSystem.heightReached = human._Transform.position.y;
        }
        else
        {
            if (human._LocomotionSystem.groundDistance >= human._LocomotionSystem.groundMaxDistance)
            {
                ...
                human._LocomotionSystem.verticalVelocity = human.GetVelocity().y;
                // apply extra gravity when falling
                if (!human._IsJumping)
                {
                    //human._Rigidbody.AddForce(human._Transform.up * human._LocomotionSystem.extraGravity * Time.fixedDeltaTime, ForceMode.VelocityChange);
                }
            }
    ...
    private static void CheckGroundDistance(Humanoid human)
    {
        if (human._LocomotionSystem._defaultCollider != null)
        {
            ...
            float dist = 10f;
            // ray for RayCast
            Ray ray2 = new Ray(human._Transform.position + new Vector3(0, human._LocomotionSystem.colliderHeight / 2, 0), Vector3.down);
            ...
            if (Physics.Raycast(ray2, out human._LocomotionSystem.groundHit, (human._LocomotionSystem.colliderHeight / 2) + dist, human._LocomotionSystem.groundLayer) && !human._LocomotionSystem.groundHit.collider.isTrigger)
                dist = human._Transform.position.y - human._LocomotionSystem.groundHit.point.y;
            // sphere cast around the base of the capsule to check the ground distance
            if (dist >= human._LocomotionSystem.groundMinDistance)
            {
                Vector3 pos = human._Transform.position + Vector3.up * (human._LocomotionSystem._defaultCollider.radius);
                ...
                if (Physics.SphereCast(ray, radius, out human._LocomotionSystem.groundHit, human._LocomotionSystem._defaultCollider.radius + human._LocomotionSystem.groundMaxDistance, human._LocomotionSystem.groundLayer) && !human._LocomotionSystem.groundHit.collider.isTrigger)
                {
                    ...
                    float newDist = human._Transform.position.y - human._LocomotionSystem.groundHit.point.y;
        ...
        if (human._LocomotionSystem._defaultCollider != null)
        {
            ...
            float dist = 10f;
            // ray for RayCast
            Ray ray2 = new Ray(human._Transform.position + new Vector3(0, human._LocomotionSystem.colliderHeight / 2, 0) + human.GetVelocity().normalized * 0.15f, Vector3.down);
            // raycast for check the ground distance
            if (Physics.Raycast(ray2, out human._LocomotionSystem.forwardGroundHit, (human._LocomotionSystem.colliderHeight / 2) + dist, human._LocomotionSystem.groundLayer) && !human._LocomotionSystem.forwardGroundHit.collider.isTrigger)
                dist = (human._Transform.position + human.GetVelocity().normalized * 0.15f).y - human._LocomotionSystem.forwardGroundHit.point.y;
            // sphere cast around the base of the capsule to check the ground distance
            if (dist >= human._LocomotionSystem.groundMinDistance)
            {
                Vector3 pos = human._Transform.position + Vector3.up * (human._LocomotionSystem._defaultCollider.radius) + human.GetVelocity().normalized * 0.15f;
                Ray ray = new Ray(pos, -Vector3.up);
                if (Physics.SphereCast(ray, radius, out human._LocomotionSystem.forwardGroundHit, human._LocomotionSystem._defaultCollider.radius + human._LocomotionSystem.groundMaxDistance, human._LocomotionSystem.groundLayer) && !human._LocomotionSystem.forwardGroundHit.collider.isTrigger)
                {
                    ...
                    float newDist = (human._Transform.position + human.GetVelocity().normalized * 0.15f).y - human._LocomotionSystem.forwardGroundHit.point.y;
                    if (dist > newDist) dist = newDist;
                }
            }
        }
    ...
    private static void MoveCharacter(Vector3 direction, Humanoid human)
    {
        ...
        if (direction.sqrMagnitude > 0.05f)
        {
            if (human._MovementState is SwimMoveState)
                human.SetVelocity(new Vector3(targetVelocity.x, human.GetVelocity().y, targetVelocity.z), true);
            else
                human.SetVelocity(targetVelocity, false);
        }
        else if (direction.sqrMagnitude < 0.05f)
            human.SetVelocity(new Vector3(0f, human.GetVelocity().y, 0f), false);
    }
    ...
    public static void CheckSlopeLimit(Humanoid human)
    {
        ...
        if (Physics.Linecast(human._Transform.position + Vector3.up * (human._LocomotionSystem._defaultCollider.height * 0.5f), human._Transform.position + normalizedMoveDir * (human._LocomotionSystem._defaultCollider.radius + 0.2f), out hitinfo, human._LocomotionSystem.groundLayer))
        {
            hitAngle = Vector3.Angle(Vector3.up, hitinfo.normal);

            Vector3 targetPoint = hitinfo.point + normalizedMoveDir * human._LocomotionSystem._defaultCollider.radius;
            if ((hitAngle > human._LocomotionSystem.slopeLimit) && Physics.Linecast(human._Transform.position + Vector3.up * (human._LocomotionSystem._defaultCollider.height * 0.5f), targetPoint, out hitinfo, human._LocomotionSystem.groundLayer))
            {
                hitAngle = Vector3.Angle(Vector3.up, hitinfo.normal);
    ...
    private static void RotateToDirection(Vector3 direction, Humanoid human, float speed = 0f)
    {
        ...
        if (human._IsStrafing)
            desiredForward = Vector3.RotateTowards(human._Transform.forward, direction.normalized, speed * Time.fixedDeltaTime, 1f);
        else
        ...
        {
            if (Physics.Raycast(human._Transform.position + Vector3.up * 5f, -Vector3.up, out RaycastHit hit, 20f, LayerMask.GetMask("Terrain")))
                up = hit.normal;
        }
        ...
        if (human._IsStrafing)
            human._Transform.rotation = lookRot;
        else
            human._Transform.rotation = Quaternion.Lerp(human._Transform.rotation, lookRot, Time.fixedDeltaTime * speed);
    }
