...
public class ItemHandleData
{
    ...
    public AsyncOperationHandle<GameObject>? _MeshSpawnHandle;
    public AsyncOperationHandle<GameObject>? _ColliderSpawnHandle;
    public string _NameForEnvironmentMeshes { get; set; }
    public CarriableObject _CarriableObjectReferance => _MeshSpawnHandle.HasValue ? _MeshSpawnHandle.Value.Result?.GetComponent<CarriableObject>() : null;
}
public abstract class ItemDefinition
{
    ...
    public AssetReferenceGameObject _AssetRefMesh;
    public AssetReferenceGameObject _AssetRefCollider;
    ...
    public ItemDefinition(string name, float weight, bool isBig, bool canBeEquipped)
    {
        ...
        _AssetRefMesh = GameManager._Instance._NameToPrefabMesh[_Name];
        _AssetRefCollider = GameManager._Instance._NameToPrefabCollider[_Name];
    }
}
...
public class Item
{
    ...
    {
        equippableItem._SpawnedHandle = _ItemDefinition._AssetRefMesh.InstantiateAsync(parentTransform);
    ...
    {
        if (_ItemHandleData._MeshSpawnHandle.HasValue && _ItemHandleData._MeshSpawnHandle.Value.IsValid()) return;
...
public class CarryItem : Item, ICanBeEquipped
{
    ...
    public AsyncOperationHandle<GameObject>? _SpawnedHandle { get => _ItemHandleData?._MeshSpawnHandle; set { _ItemHandleData._MeshSpawnHandle = value; } }
    ...
    {
        while (_ItemHandleData._MeshSpawnHandle?.Result == null) { Debug.LogError("Backpack handle is null"); yield return null; }
        float stretch = (1f - normalizedFullness) * 80f;
        Transform backpack = _ItemHandleData._MeshSpawnHandle.Value.Result.transform;
