...
public class LocomotionState : MovementState
{
    ...
    public void EnterState(MovementState oldState)
    {
        ...
        _human._LocomotionSystem.EnableOrDisableDefaultCollider(true);
    ...
    {
        _human._LocomotionSystem.EnableOrDisableDefaultCollider(false);
...
public class CrouchMoveState : MovementState
{
    ...
    {
        _stateChangeCounter = 0.1f;
        _human._LocomotionSystem.EnableOrDisableCrouchCollider(true);
    ...
    public void ExitState(MovementState newState)
    {
        ...
        _human._LocomotionSystem.EnableOrDisableCrouchCollider(false);
    }
    public void DoState()
    {
        if (_human._HealthSystem._IsUnconscious)
        ...
        if (_stateChangeCounter > 0f)
            _human.SetVelocity(Vector3.Lerp(_human.GetVelocity(), Vector3.zero, Time.deltaTime * 2f), false);
        else
            MovementStateMethods.UpdateMoveDirection(_human, GameManager._Instance._MainCamera.transform);
...
public class ProneMoveState : MovementState
{
    ...
    {
        _stateChangeCounter = 0.1f;
        _human._LocomotionSystem.EnableOrDisableProneCollider(true);
        _human._IsSprinting = false;
    ...
    public void ExitState(MovementState newState)
    {
        _human._LocomotionSystem.EnableOrDisableProneCollider(false);
...
public class SwimMoveState : MovementState
{
    ...
    public void FixedUpdate()
    {
        ...
        float targetYVel = (WorldHandler._Instance._SeaLevel - 1.5f - _human._Transform.position.y) * 7.5f;
        _human.SetVelocity(new Vector3(_human.GetVelocity().x, Mathf.Lerp(_human.GetVelocity().y, targetYVel, Time.deltaTime * 3f), _human.GetVelocity().z), true);
    }
}
...
public class UnconsciousMoveState : MovementState
{
    ...
    public void EnterState(MovementState oldState)
    {
        ...
        Vector3 currentVel = _human.GetVelocity();
        _human.OpenOrCloseMainPhysics(true);
    ...
    private IEnumerator GetUpCoroutine()
    {
        ...
        _human.EnterState(new LocomotionState(_human));
        _human.OpenOrCloseMainPhysics(false);
    }
    private void ArrangeHumanoidToHips(Transform hips, Vector3 targetPos, Quaternion targetRot)
    {
        ...
        _human._Transform.Rotate(0f, -deltaY, 0f, Space.World);
    }
}
#endregion
...
public class RangedWeaponHandState : HandState
{
    ...
    public void EnterState(HandState oldState)
    {
        ...
        if (_human is Player player)
            GameManager._Instance.BufferActivated(ref player._AttackReadyBuffer, player, ref player._AttackReadyCoroutine);
        _human._AimPosition = _human._Transform.forward;
    }
