...
public class LocomotionSystem : MonoBehaviour
{
    ...
    public void EnableOrDisableDefaultCollider(bool value)
    {
        _defaultCollider.enabled = value;
        if (value)
            _human._EntityPhysicObject.SetActiveCollider(0);
    }
    public void EnableOrDisableCrouchCollider(bool value)
    {
        _crouchCollider.GetComponent<Collider>().enabled = value;
        if (value)
            _human._EntityPhysicObject.SetActiveCollider(1);
    }
    public void EnableOrDisableProneCollider(bool value)
    {
        _proneCollider.GetComponent<Collider>().enabled = value;
        if (value)
            _human._EntityPhysicObject.SetActiveCollider(2);
    }
    ...
    public virtual void UpdateAnimator()
    {
        ...
        {
            Vector3 flatSpeed = _human.GetVelocity();
            flatSpeed.y = 0f;
